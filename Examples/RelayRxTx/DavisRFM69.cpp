...
volatile uint32_t DavisRFM69::txPeriod;
volatile uint32_t DavisRFM69::lastTx;
...
void DavisRFM69::initialize(byte freqBand)
{
  ...
  Timer1.initialize(500); // periodical interrupts every 1/2 ms for missed packet detection and other checks
...
void DavisRFM69::handleTimerInt() {
  ...
  // Is it time to transmit? unsigned wrap around safe
  if (lastRx - lastTx >= txPeriod) {
    sendNextPacket(lastRx);
    lastTx = lastRx;
    return;
  }
